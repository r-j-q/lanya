<template>
	<cover-view class="backgroundColorLine">


		<map id="map" ref="map"></map>

			<cover-view class="backgroundColorLine">
				<cover-view class="getLocation" @click="handleLanya">
					<!-- <cover-image :src="getLocation" class="getLocation-"></cover-image> -->
					{{address ||"定位失败，点击重新定位"}}
					{{ latitude || '9'   }}{{longitude}}
				</cover-view>


				<cover-view class="backgroundColorLineText" v-if="!showButton">
					蓝牙连接中,请稍等
				</cover-view>

				<cover-view class="backgroundColorLineSuccess" v-if="showButton">
					{{ list[0].name}} 连接成功
				...
				<cover-view class="writeStyle" v-if="showButton">

					<cover-button type="primary" class="writeStyle12" @click="openBluetoothAdapterPay">开
						盒</cover-button>
					<cover-button type="primary" class="writeStyle12" @click="writeBLECharacteristicValue('PCSS')">激 活 机
						身</cover-button>
				</cover-view>
